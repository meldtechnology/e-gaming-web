...
export const DocumentReviewForm = ({application}) => {
  ...
  const canReviewApplication = checkPermission('CAN_REVIEW_APPLICATION');
  const canApproveApplication = checkPermission('CAN_APPROVE_APPLICATION');
      ...
      <Box>
        <form onSubmit={formik.handleSubmit}>
          <TextareaAutosize
            ...
            className={`w-[100%]`}
          />
          {canReviewApplication && !showCheck && !showLoader ? (
          ...
          {canApproveApplication ? (
            <Box
              ...
              margin={'0 auto'}
            >
              {!show && !(app?.declinedOn || app?.approvedOn) ? (
                <Button size={"large"}
                        variant={"contained"}
                        type={"button"}

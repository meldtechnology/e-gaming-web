    ...
    >
      {data?.map((d, index) => (
        <Row key={`dt-${index}`}>
          ...
          <Td>{d?.createdBy}</Td>
          <Td className="text-text-secondary">{d?.submittedOn}</Td>
          <Td>
            <div className="flex items-center gap-2">
              <span>{d?.amountPaid}</span>
              <StatusBadge status={d?.paymentStatus} fallback="PENDING" />
            </div>
          </Td>
          <Td>{d?.amountPayable}</Td>
          <Td><StatusBadge status={d?.status} /></Td>
          <Td>
            <IconAction onClick={() => setSelectedApplication(d)} label="Review application">

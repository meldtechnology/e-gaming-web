...
- Note: The core code fix is already present in the working tree
  (`src/ui-components/FormBuilder/index.jsx:89`, uncommitted). Tasks below focus
  on confirming, testing, hardening, and shipping that change.
...
4. [x] Confirm current state of the fix in the working tree vs. the plan's target diff (already applied, uncommitted).
...
4. [ ] Run the app (`npm run dev`) and open the Form Designer for a document with no saved `formTemplate` (Documents → Files → open designer); confirm it renders instead of crashing into `AppErrorBoundary`.
   - Terminal verification completed: Vite served successfully on `http://127.0.0.1:3001/` after port 3000 was already in use. Authenticated document data/session was not available for this manual path.
...
- Verification run:
  ...
  - `npm test`: passed, 27 files / 67 tests.
  ...
  - `npm run dev`: port 3000 was in use; app served on `http://127.0.0.1:3001/` and returned `HTTP/1.1 200 OK`.
